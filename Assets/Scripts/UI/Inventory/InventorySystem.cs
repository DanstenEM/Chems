...
public class InventorySystem : MonoBehaviour, IInitializable, IDisposable
{
    ...
    public Dictionary<InventoryItemObj, int> GetItemCounts()
    {
        var counts = new Dictionary<InventoryItemObj, int>();
        ...
        foreach (var slot in slots)
        {
            ...
            var slotItem = slot.GetComponentInChildren<InventoryItem>();
            if (slotItem == null || slotItem.itemObj == null || slotItem.count <= 0)
            {
                continue;
            }

            if (counts.TryGetValue(slotItem.itemObj, out var existingCount))
            {
                counts[slotItem.itemObj] = existingCount + slotItem.count;
            }
            else
            {
                counts[slotItem.itemObj] = slotItem.count;
            }
        }
    ...
    public bool TryConsumeOne(InventoryItemObj itemObj)
    {
        if (itemObj == null || slots == null)
        {
            return false;
        }
